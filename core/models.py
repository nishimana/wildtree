...
FullPathIndex = dict[str, KeyDefinition]
"""フルパスインデックスの型。

参照の full_path（"cards/" プレフィックスを除去済み）をキーとし、
対応する KeyDefinition を値とする。

resolver.build_full_path_index() が KeyRegistry と cards_dir から構築する。
フルパス参照の O(1) ルックアップに使用する。

例:
  "SAO/CH_asada/朝田詩乃" → KeyDefinition(name="朝田詩乃", ...)
  "メイン" → KeyDefinition(name="メイン", ...)  # cards_dir 直下のファイル
"""
